<?php
    ...
    /**
      *
      *   Defines the command to perform HTTP M(ultiple)DELETE on passed resource reference, data can be in form allowed by curl_setopt CURLOPT_DELETEFIELDS
      * 
        @file MDELETE.php
	  ...
	  * @throws Exception
	  */
	  public function multi_delete($actions = NULL, $max_handles = NULL,&$success) {
          ...
          /**
	    @brief Execute the MDELETE Command
	    @name HTTP_MDELETE_run
	    @param $success bool will be set to true if command succeed, else false
	    ...
	    @return array
	    **/
	    function HTTP_MDELETE_run(&$success,$data,$extra) {
	   //----------------------------------------------
		   $actions = $data;
		   $max_handles=$éxtra;
		   $this->multi_delete($actions, $max_handles, $success) ;
