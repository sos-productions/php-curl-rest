<?php
    ...
    /**
      *
      *   Defines the command to perform HTTP M(ultiple)POST on passed resource reference, data can be in form allowed by curl_setopt CURLOPT_POSTFIELDS
      * 
        @file MPOST.php
	  ...
	  * @throws Exception
	  */
	  public function multi_post($actions = NULL, $data = NULL, $max_handles = NULL,&$success) {
          ...
          /**
	    @brief Execute the MPOST Command
	    @name HTTP_MPOST_run
	    ...
	    @return array
	    **/
	    function HTTP_MPOST_run(&$success,$data,$extra,$conf) {
	   //----------------------------------------------
		   $actions = $data; 
		   $data = $extra; 
		   $max_handles=$conf;
		   $this->multi_post($actions, $data, $max_handles, $success) ;
